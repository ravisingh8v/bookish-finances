...
begin
  ...
  if not found or auth.uid() <> d.created_by then
  ...
  email_value := case
    when _payload ? 'borrowerEmail' then nullif(lower(trim(_payload->>'borrowerEmail')), '')
    else d.counterparty_email
  end;

  if new_type not in ('one_time', 'custom', 'emi')
    or new_direction not in ('receivable', 'payable')
    or new_amount <= 0
    or new_amount < d.paid_amount then
    raise exception 'Invalid debt values';
  end if;
  ...
  set
    ...
    due_date = case
      when new_type = 'one_time' then nullif(_payload->>'dueDate', '')::date
      when new_type = 'custom' then nullif(_payload->'installments'->0->>'due_date', '')::date
      when new_type = 'emi' then nullif(_payload->'loan'->>'first_emi_date', '')::date
      else due_date
    end,
    total_amount = new_amount,
    remaining_amount = new_amount - paid_amount,
    status = case
      when new_amount = paid_amount then 'paid'
  ...
  if d.paid_amount = 0 then
    ...
    if new_type = 'custom' then
      ...
      if n < 2 or abs((select coalesce(sum(amount), 0) from public.debt_installments where debt_id = _debt_id) - new_amount) > 0.01 then
        raise exception 'Installments must equal debt total';
      end if;
    elsif new_type = 'emi' then
      loan := _payload->'loan';
      emi_count := (loan->>'number_of_emis')::integer;
      emi_amount := (loan->>'emi_amount')::numeric;
  ...
  values(_debt_id, 'updated', auth.uid());
end
$function$;
